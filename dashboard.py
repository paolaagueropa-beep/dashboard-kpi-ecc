...
def cargar_datos():
    ...
    resumen   = pd.read_excel(contenido, sheet_name="Resumen_Agentes")
    contenido.seek(0)
    jefatura  = pd.read_excel(contenido, sheet_name="Resumen_Jefatura")
    contenido.seek(0)
    jp_semana = pd.read_excel(contenido, sheet_name="JP_Semana")
    contenido.seek(0)
    jp_dia    = pd.read_excel(contenido, sheet_name="JP_Dia")
    contenido.seek(0)
    historico = pd.read_excel(contenido, sheet_name="Historico")
    contenido.seek(0)
    criticos  = pd.read_excel(contenido, sheet_name="Agentes_Criticos")
    contenido.seek(0)
    hist_ag   = pd.read_excel(contenido, sheet_name="Historico_Agente")
    contenido.seek(0)
    semanal   = pd.read_excel(contenido, sheet_name="Resumen_Semanal")
    contenido.seek(0)
    diario    = pd.read_excel(contenido, sheet_name="Detalle_Diario")
    return resumen, jefatura, jp_semana, jp_dia, historico, criticos, hist_ag, semanal, diario
...
with st.spinner("⏳ Cargando datos desde Drive..."):
    try:
        resumen, jefatura, jp_semana, jp_dia, historico, criticos, hist_ag, semanal, diario = cargar_datos()
